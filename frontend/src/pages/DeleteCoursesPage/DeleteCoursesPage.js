...
const DeleteCoursesPage = () => {
  ...
  const [formData, handleInputChange, handleSubmit] = useCustomForm(initialValues, postDeletedCourses);

  async function postDeletedCourses() {
    let courseId = formData.course_id
    { console.log('courseId', courseId) }
    try {
      let response = await axios.delete(`http://127.0.0.1:8000/api/student_courses/grade_course_object/${courseId}/`, formData, {
        headers: {
          Authorization: "Bearer " + token,
        },
      });
      navigate(`/deleted_courses/${studentId}`);
      console.log('post deleted Course', formData)
    } catch (error) {
